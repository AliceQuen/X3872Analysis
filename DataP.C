...
void DataP::Loop(TString outputname)
{
        ...
        float Jpsi1_mass_diff;
        vars.push_back(&Jpsi1_mass_diff);
        SourceTree->Branch("Jpsi1_mass_diff", &Jpsi1_mass_diff);
        float Jpsi2_mass;
        ...
        for (Long64_t jentry = 0; jentry < nentries; jentry++)
        {
                ...
                nbytes += nb;
                if (!MatchJpsiTrigNames->size())
                        continue;
                // Multi-candidate handling
                final_candidates.clear();
                unsigned int X_JPiPi_size = X_JPiPi_mass->size();
                for (unsigned int m = 0; m < X_JPiPi_size; m++)
                        final_candidates.push_back(m);
                bool re_i = false;
                bool re_j = false;
                ...
                for (auto k = final_candidates.begin(); k != final_candidates.end(); k++)
                {
                        ...
                        if ((*muJpsiFilterRes)[(*X_mu1Id)[l]])
                                filter++;
                        if ((*muJpsiFilterRes)[(*X_mu2Id)[l]])
                                filter++;
                        if ((*muJpsiFilterRes)[(*X_mu3Id)[l]])
                                filter++;
                        if ((*muJpsiFilterRes)[(*X_mu4Id)[l]])
                                filter++;
                        ...
                        if (!(PSI2S_MASS_MIN <= Psi2S_mass && Psi2S_mass <= PSI2S_MASS_MAX))
                                continue;
                        Jpsi1_mass_diff = fabs(Jpsi1_P4.M() - JPSI_MASS);
